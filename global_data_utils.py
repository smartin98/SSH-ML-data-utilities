...
def extract_tracked(data_tracked, L_x, L_y, lon0, lat0, transformer_ll2xyz, nrt):
    ds = data_tracked
...
def grid_sst_hr(data_sst_hr, n, L_x, L_y, lon0, lat0, coord_grid):
    ds = data_sst_hr
...
def sst_lr_delaunay(data_sst_lr, n, L_x, L_y, lon0, lat0):
    ds = data_sst_lr
    buffer = 0.05e6 # buffer to avoid exluding data due to inverse stereographic (rather than orthographic) projection
    boxx, boxy = box([-L_x/2-buffer,L_x/2+buffer],[-L_y/2-buffer,L_y/2+buffer])
    alt0 = 0
...
def grid_sst_lr(data_sst_lr, n, L_x, L_y, lon0, lat0,tri):
    ds = data_sst_lr
    buffer = 0.05e6 # buffer to avoid exluding data due to inverse stereographic (rather than orthographic) projection
    boxx, boxy = box([-L_x/2-buffer,L_x/2+buffer],[-L_y/2-buffer,L_y/2+buffer])
    alt0 = 0
...
def grid_coords(data_bath, n, L_x, L_y, lon0, lat0):
    ds = data_bath
